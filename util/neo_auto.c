                         ...
                         int *do_free)
{
  ...
  if (st == HTML_STATE_CSS_FILE || (tag && strcmp(tag, "style") == 0)) {
...
NEOERR *neos_auto_parse(NEOS_AUTO_CTX *ctx, const char *str, int len)
{
  ...
  /* TODO(mugdha): Add a check for return value HTML_STATE_ERROR when it is
     available */
